# 徳島 ベビー用品チラシチェッカー ver 2.14

GitHub → Vercel 配備用のNext.jsアプリです。

## V2.14追加変更

- `Page.captureScreenshot` が失敗するビューアー向けに、Canvas直接保存・画像直接DL・固定ビューポート分割撮影の多段フォールバックを追加しました。
- コストコオンラインは指定2ページ内で、赤文字かつ「引き後」を含む表示のある商品を抽出します。
...
GitHubリポジトリの中身をV2.14で上書きしてCommit/Pushしてください。VercelのGit連携が有効なら自動再デプロイされます。


## V2.14

- 全社Shufoo!検索を廃止し、指定された公式URLのみ使用します。
- 更新開始時に旧チラシBlobと現在表示用キャッシュを削除してから取り直します（履歴CSVは残ります）。
- スクリーンショット前に画像/Canvasの描画完了を待つようにしました。
- 更新中の進捗欄に「この店舗をスキップ」を追加しました。
- コストコオンラインは指定2URLを情報元1/2として表示し、赤文字の「引き後」を含む商品を抽出します。


## V2.14
バースデイの実クリック取得、Blob表示改善、コストコ商品画像表示、コストコ抽出方法非表示を追加しました。
