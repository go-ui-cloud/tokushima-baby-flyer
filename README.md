...
V2.10では「チラシっぽい画像を探す」方式より先に、指定された公式店舗ページで店舗ごとのクリック手順を実行します。クリック先のチラシ/ビューアーをスクリーンショットし、その画像をOCRします。指定手順で取得できなかった場合のみ従来の公式ページ探索 → Shufoo!等のフォールバックへ進みます。
...
通常商品は表示せず、ページ内に文字列として **`¥○○引き後`** または **`￥○○引き後`** が確認できる対象カテゴリ商品のみ表示します。値引額の表記は画面でも `¥1,000引き後` の形式で表示します。
...
GitHubリポジトリの中身をV2.10で上書きしてCommit/Pushしてください。VercelのGit連携が有効なら自動再デプロイされます。
