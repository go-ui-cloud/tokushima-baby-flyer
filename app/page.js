...
export default function Home(){
  ...
  return <main>
    <header className="topbar">
      <div className="heroCopy"><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><span className="heroIcon">🍼</span><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 3.0.1</span></div><p className="sub">徳島の各店舗で見つけたベビー用品の安売り情報を手動で登録・一覧表示します。コストコオンラインだけは公式ページから自動更新します。</p></div>
    ...
    <section className="storeList">{visibleResults.map(store=>{
      ...
      return <article className="store" key={store.id}>
        ...
        {store.id==='costco-online'?<><div className="storeStatusRow"><span className={`freshness ${tone}`}>📅 {store.flyerFreshness||'最新性不明'}</span>{store.durationMs!=null&&<span className="metaChip">⏱ {(store.durationMs/1000).toFixed(1)}秒</span>}</div>{store.error&&<p className="error">❌ 取得エラー: {store.error}</p>}{(store.warnings||[]).length>0&&<p className="warning storeWarning">⚠️ {store.warnings.slice(0,3).join(' / ')}</p>}</>:admin.authenticated&&<details className="manualFormBox"><summary>＋ この店舗に商品を追加</summary><form className="manualForm" onSubmit={e=>addManualItem(e,store.id)}><label>商品名 <b>必須</b><input name="product" required maxLength="120"/></label><label>商品画像<input name="image" type="file" accept="image/*"/></label><label>価格 <b>必須</b><input name="price" required maxLength="40" placeholder="例：1,280円"/></label><label>広告開始日<input name="startDate" type="date"/></label><label>広告終了日<input name="endDate" type="date"/></label><label>カテゴリ <b>必須</b><select name="category" required defaultValue=""><option value="" disabled>選択してください</option>{CATEGORY_ORDER.map(x=><option key={x} value={x}>{x}</option>)}</select></label><label className="manualMemo">メモ<textarea name="memo" rows="3" maxLength="500"/></label><button type="submit" disabled={savingStore===store.id}>{savingStore===store.id?'登録中…':'商品を登録'}</button></form></details>}
