...
export default function Home(){
  ...
  return <main>
    <header className="topbar"><div><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 2.13</span></div><p className="sub">指定された各社の公式URLだけを使用し、Shufoo!では検索しません。取得状況を「チラシを発見」「OCRを実行中」などリアルタイム表示します。最新性は掲載側の日付とチラシ内の日付の2段階で確認します。コストコオンラインは指定した2ページ内で、赤文字の「引き後」が確認できる商品をカテゴリ判定なしで表示します。ベビー服は対象外です。</p></div><div className="actions"><a className="ghostButton" href="/api/history.csv">CSV履歴</a><button className="updateButton" onClick={update} disabled={loading}>{loading?'解析中…':'↻ 最新情報に更新'}</button></div></header>
    ...
    <section className="storeList">{visibleResults.map(store=>{const items=(store.items||[]).filter(x=>visibleCategories.has(x.category));return <article className="store" key={store.id}><div className="storeHead"><div><div className="titleRow"><h2>{store.chain}</h2><span className="area">{store.area}</span></div><p className="stores">対象: {STORE_NAMES[store.id]||`${store.chain} ${store.area}`}</p></div><div className="sourceLinks">{store.id==='costco-online'&&store.sourceUrls?.length?store.sourceUrls.map((src,i)=><a key={src.url} href={src.url} target="_blank" rel="noreferrer">情報元{i+1}</a>):<a href={store.sourceUrl} target="_blank" rel="noreferrer">情報元</a>}{(store.flyers||[]).slice(0,6).map((f,i)=><a key={`${f.url}-${i}`} href={f.viewerUrl||f.url} target="_blank" rel="noreferrer" title={`掲載側: ${f.sourceDateCheck?.raw||'不明'} / チラシ内: ${f.dateCheck?.raw||'不明'}`}>チラシ{(store.flyers||[]).length>1?i+1:''}{f.verification?.label?`・${f.verification.label}`:''}</a>)}</div></div>{store.error&&<p className="error">取得エラー: {store.error}</p>}{!store.error&&store.durationMs!=null&&<p className="storeMeta">前回処理時間: {(store.durationMs/1000).toFixed(1)}秒 ／ チラシ取得元: {store.sourceProvider||'不明'} ／ 最新性: {store.flyerFreshness||'不明'}{store.extendedAnalysis?' ／ 5分モード':''}</p>}{(store.warnings||[]).length>0&&<p className="warning storeWarning">{store.warnings.slice(0,3).join(' / ')}</p>}{!items.length?<div className="empty">{store.id==='costco-online'?'指定したコストコオンライン2ページで赤文字の「引き後」がある商品を確認できませんでした。':'現在の最新チラシ内で、表示対象カテゴリのベビー用品を確認できませんでした。'}</div>:<div className="cards">{items.map((x,i)=><div className="card" key={`${x.product}-${i}`}><div className="icon">{icons[x.category]||'🧺'}</div><span className="cat">{x.category}</span><h3>{x.product}</h3>{x.discountAfter&&<div className="discountAfter">{x.discountAfter}</div>}<div className="price">{x.price}</div><dl><div><dt>販売開始日</dt><dd>{x.startDate}</dd></div><div><dt>終了日</dt><dd>{x.endDate}</dd></div><div><dt>抽出方法</dt><dd>{x.confidence}</dd></div></dl><a className="detailLink" href={x.flyerUrl!=='不明'?x.flyerUrl:x.sourceUrl} target="_blank" rel="noreferrer">情報を確認 →</a></div>)}</div>}</article>;})}</section>
  </main>;
}
