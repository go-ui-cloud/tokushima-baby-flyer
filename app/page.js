...
const STORE_IDS=['nishimatsuya','birthday-aizumi','akachan-aizumi','direx','doramori','cosmos','lady','aoki','donki','costco'];
const STORE_NAMES={
...
export default function Home(){
  ...
  return <main>
    <header className="topbar"><div><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><h1>ベビー用品 セールチェッカー</h1><span className="versionBadge">ver 2.4</span></div><p className="sub">最新チラシ・公開WEBクーポンから、値引き・特価など販促性の高いベビー用品だけを抽出します。ベビー服は対象外です。</p></div><div className="actions"><a className="ghostButton" href="/api/history.csv">CSV履歴</a><button className="updateButton" onClick={update} disabled={loading}>{loading?'解析中…':'↻ 最新情報に更新'}</button></div></header>
    <section className="summary"><div><span>最終更新</span><strong>{data.updatedAt?fmtDate(data.updatedAt):'未更新'}</strong></div><div><span>履歴保存</span><strong>{data.persistence?.database?'DB':'未設定'}</strong></div><div><span>チラシ保存</span><strong>{data.persistence?.blob?'Vercel Blob':'元URLのみ'}</strong></div><div><span>抽出方針</span><strong>セール品のみ</strong></div></section>
