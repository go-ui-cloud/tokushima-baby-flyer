...
export default function Home(){
  ...
  return <main>
    <header className="topbar"><div><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 2.10</span></div><p className="sub">指定店舗の公式チラシを最優先。取得状況を「チラシを発見」「OCRを実行中」などリアルタイム表示します。最新性は掲載側の日付とチラシ内の日付の2段階で確認します。コストコオンラインは指定した2カテゴリ内で「¥○○引き後」が明記された対象商品のみ表示します。ベビー服は対象外です。</p></div><div className="actions"><a className="ghostButton" href="/api/history.csv">CSV履歴</a><button className="updateButton" onClick={update} disabled={loading}>{loading?'解析中…':'↻ 最新情報に更新'}</button></div></header>
