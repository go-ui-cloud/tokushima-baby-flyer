...
const STORE_IDS=['nishimatsuya','birthday-aizumi','akachan-aizumi','direx','doramori','cosmos','lady','aoki','donki'];
const STORE_NAMES={
  'nishimatsuya':'西松屋 徳島南矢三店','birthday-aizumi':'バースデイ 藍住店','akachan-aizumi':'アカチャンホンポ ゆめタウン徳島店','direx':'ダイレックス 田宮店','doramori':'ドラッグストアモリ 徳島住吉店','cosmos':'ドラッグコスモス 住吉店','lady':'レデイ薬局 田宮街道店','aoki':'クスリのアオキ 北島田店','donki':'MEGAドン・キホーテ徳島店'
};
...
export default function Home(){
  ...
  return <main>
    ...
    <section className="storeList">{visibleResults.map(store=>{const items=(store.items||[]).filter(x=>visibleCategories.has(x.category));return <article className="store" key={store.id}><div className="storeHead"><div><div className="titleRow"><h2>{store.chain}</h2><span className="area">{store.area}</span></div><p className="stores">対象: {(store.storeKeywords||[]).join(' / ')}</p></div><div className="sourceLinks"><a href={store.sourceUrl} target="_blank" rel="noreferrer">情報元</a>{(store.flyers||[]).slice(0,6).map((f,i)=><a key={`${f.url}-${i}`} href={f.viewerUrl||f.url} target="_blank" rel="noreferrer">チラシ{(store.flyers||[]).length>1?i+1:''}</a>)}</div></div>{store.error&&<p className="error">取得エラー: {store.error}</p>}{!store.error&&store.durationMs!=null&&<p className="storeMeta">前回処理時間: {(store.durationMs/1000).toFixed(1)}秒 ／ チラシ取得元: {store.sourceProvider||'不明'}</p>}{(store.warnings||[]).length>0&&<p className="warning storeWarning">{store.warnings.slice(0,3).join(' / ')}</p>}{!items.length?<div className="empty">現在の表示条件に合う特価・セール対象のベビー用品を確認できませんでした。</div>:<div className="cards">{items.map((x,i)=><div className="card" key={`${x.product}-${i}`}><div className="icon">{icons[x.category]||'🧺'}</div><span className="cat">{x.category}</span><h3>{x.product}</h3><div className="price">{x.price}</div><dl><div><dt>販売開始日</dt><dd>{x.startDate}</dd></div><div><dt>終了日</dt><dd>{x.endDate}</dd></div><div><dt>抽出方法</dt><dd>{x.confidence}</dd></div></dl><a className="detailLink" href={x.flyerUrl!=='不明'?x.flyerUrl:x.sourceUrl} target="_blank" rel="noreferrer">情報を確認 →</a></div>)}</div>}</article>;})}</section>
  </main>;
}
