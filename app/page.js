...
export default function Home(){
  ...
  async function update(){
    ...
    try{
      setMessage('保存済みチラシ画像を削除し、取得キャッシュをクリアしています…');
      const prep=await fetch('/api/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'prepare',batchId})});if(!prep.ok){const j=await prep.json().catch(()=>({}));throw new Error(j.error||'キャッシュクリアに失敗しました');}
      let snapshot={updatedAt:null,results:[],persistence:data.persistence||{}};setData(snapshot);let failed=0,skipped=0;const failedNames=[],skippedNames=[];
      for(let i=0;i<ids.length;i++){
        const started=Date.now();currentStoreRef.current=ids[i];skipRequestedRef.current=false;setElapsed(0);setProgress({index:i+1,total:ids.length,storeId:ids[i],store:STORE_NAMES[ids[i]]||ids[i],phase:'開始',detail:'更新処理を開始しています'});setMessage(`最新チラシを取得・解析中… ${i+1}/${ids.length} ${STORE_NAMES[ids[i]]||ids[i]}`);
        ...
        try{const controller=new AbortController();currentAbortRef.current=controller;const abortTimer=setTimeout(()=>controller.abort(),300000);const res=await fetch('/api/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storeId:ids[i],batchId}),signal:controller.signal});clearTimeout(abortTimer);const json=await res.json();if(!res.ok)throw new Error(json.error||`${ids[i]} の更新に失敗しました`);const current=[...(snapshot.results||[])];const idx=current.findIndex(r=>r.id===json.result.id);if(idx>=0)current[idx]=json.result;else current.push(json.result);snapshot={...snapshot,results:current,persistence:json.persistence||snapshot.persistence};setData(snapshot);if(json.result?.error){failed++;failedNames.push(STORE_NAMES[ids[i]]||ids[i]);}else if(json.result?.skipped){skipped++;skippedNames.push(STORE_NAMES[ids[i]]||ids[i]);}}
        catch(e){if(skipRequestedRef.current){skipped++;skippedNames.push(STORE_NAMES[ids[i]]||ids[i]);setMessage(`${STORE_NAMES[ids[i]]||ids[i]} をスキップしました。次の店舗へ進みます…`);}else{failed++;failedNames.push(STORE_NAMES[ids[i]]||ids[i]);setMessage(`${STORE_NAMES[ids[i]]||ids[i]} は失敗/タイムアウト。次の店舗へ進みます… (${e.name==='AbortError'?'最大300秒タイムアウト':e.message})`);}}finally{currentAbortRef.current=null;clearInterval(timer);clearInterval(progressPoll);}
      }
      const finalRes=await fetch('/api/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'finalize',batchId,snapshot})});const finalJson=await finalRes.json();if(!finalRes.ok)throw new Error(finalJson.error||'履歴保存に失敗しました');setData(finalJson);setMessage(`更新完了：${ids.length-failed-skipped}店舗成功 / ${skipped}店舗スキップ${skippedNames.length?`（${skippedNames.join('、')}）`:''} / ${failed}店舗失敗${failedNames.length?`（${failedNames.join('、')}）`:''}。履歴を保存しました`);
  ...
  return <main>
    <header className="topbar"><div><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 2.15</span></div><p className="sub">指定された各社の公式URLだけを使用し、Shufoo!では検索しません。取得状況を「チラシを発見」「OCRを実行中」などリアルタイム表示します。最新性は掲載側の日付とチラシ内の日付の2段階で確認します。コストコオンラインは指定した2ページ内で、赤文字の「引き後」が確認できる商品を画像付きで表示します。ベビー服は対象外です。</p></div><div className="actions"><a className="ghostButton" href="/api/history.csv">CSV履歴</a><button className="updateButton" onClick={update} disabled={loading}>{loading?'解析中…':'↻ 最新情報に更新'}</button></div></header>
    ...
    <section className="storeList">{visibleResults.map(store=>{const items=(store.items||[]).filter(x=>visibleCategories.has(x.category));return <article className="store" key={store.id}><div className="storeHead"><div><div className="titleRow"><h2>{store.chain}</h2><span className="area">{store.area}</span></div><p className="stores">対象: {STORE_NAMES[store.id]||`${store.chain} ${store.area}`}</p></div><div className="sourceLinks">{store.id==='costco-online'&&store.sourceUrls?.length?store.sourceUrls.map((src,i)=><a key={src.url} href={src.url} target="_blank" rel="noreferrer">情報元{i+1}</a>):<a href={store.sourceUrl} target="_blank" rel="noreferrer">情報元</a>}{(store.flyers||[]).filter(f=>f.viewerUrl||/^https?:/i.test(f.url||'')).slice(0,6).map((f,i)=><a key={`${f.url}-${i}`} href={f.viewerUrl||f.url} target="_blank" rel="noreferrer" title={`掲載側: ${f.sourceDateCheck?.raw||'不明'} / チラシ内: ${f.dateCheck?.raw||'不明'}`}>チラシ{(store.flyers||[]).length>1?i+1:''}{f.verification?.label?`・${f.verification.label}`:''}</a>)}</div></div>{store.error&&<p className="error">取得エラー: {store.error}</p>}{!store.error&&store.durationMs!=null&&<p className="storeMeta">前回処理時間: {(store.durationMs/1000).toFixed(1)}秒 ／ チラシ取得元: {store.sourceProvider||'不明'} ／ 最新性: {store.flyerFreshness||'不明'}{store.extendedAnalysis?' ／ 5分モード':''}</p>}{(store.warnings||[]).length>0&&<p className="warning storeWarning">{store.warnings.slice(0,3).join(' / ')}</p>}{!items.length?<div className="empty">{store.id==='costco-online'?'指定したコストコオンライン2ページで赤文字の「引き後」がある商品を確認できませんでした。':'現在の最新チラシ内で、表示対象カテゴリのベビー用品を確認できませんでした。'}</div>:<div className="cards">{items.map((x,i)=><div className={`card ${store.id==='costco-online'?'costcoCard':''}`} key={`${x.product}-${i}`}>{store.id==='costco-online'&&x.imageUrl?<img className="costcoThumb" src={x.imageUrl} alt={x.product} loading="lazy" referrerPolicy="no-referrer"/>:<div className="icon">{icons[x.category]||'🧺'}</div>}<span className="cat">{x.category}</span><h3>{x.product}</h3>{x.discountAfter&&<div className="discountAfter">{x.discountAfter}</div>}<div className="price">{x.price}</div><dl><div><dt>販売開始日</dt><dd>{x.startDate}</dd></div><div><dt>終了日</dt><dd>{x.endDate}</dd></div>{store.id!=='costco-online'&&<div><dt>抽出方法</dt><dd>{x.confidence}</dd></div>}</dl><a className="detailLink" href={x.flyerUrl!=='不明'?x.flyerUrl:x.sourceUrl} target="_blank" rel="noreferrer">情報を確認 →</a></div>)}</div>}</article>;})}</section>
  </main>;
}
