...
function fmtStoreUpdate(v){if(!v)return'未更新';const d=new Date(v);return Number.isNaN(d.getTime())?'未更新':d.toLocaleString('ja-JP',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});}
...
function productPageUrl(item){return item.flyerUrl&&item.flyerUrl!=='不明'?item.flyerUrl:item.sourceUrl;}
...
export default function Home(){
  ...
  return <main>
    <header className="topbar">
      <div className="heroCopy"><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><span className="heroIcon">🍼</span><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 3.3.8</span></div><p className="sub">徳島の各店舗で見つけたベビー用品の安売り情報を手動で登録・一覧表示します。オンライン4店舗は公式ページから店舗を選んで更新できます。</p></div>
    ...
    <section className="storeList">{visibleResults.map(store=>{
      ...
      return <article className={`store store-${store.id}`} key={store.id}>
        <div className="storeHead"><div className="storeIdentity"><div className="storeIconBox">{STORE_ICONS[store.id]||'🏪'}</div><div><div className="titleRow"><h2>{store.chain}</h2><span className="area">{store.area}</span><span className="storeUpdated">更新日 {fmtStoreUpdate(store.checkedAt)}</span></div><p className="stores">対象: {STORE_NAMES[store.id]||`${store.chain} ${store.area}`}</p></div></div>
          <div className="sourceLinks">{isAutomatic&&store.sourceUrls?.length?store.sourceUrls.slice(0,1).map(src=><a key={src.url} href={src.url} target="_blank" rel="noreferrer">🔗 情報元（{src.label}）</a>):<a href={store.sourceUrl} target="_blank" rel="noreferrer">🔗 情報元</a>}</div>
        </div>
