...
export default function Home(){
  ...
  async function update(){
    ...
    try{
      ...
      let snapshot={updatedAt:null,results:[],persistence:data.persistence||{}};setData(snapshot);let failed=0,skipped=0;
      for(let i=0;i<ids.length;i++){
        ...
        catch(e){if(skipRequestedRef.current){skipped++;setMessage(`${STORE_NAMES[ids[i]]||ids[i]} をスキップしました。次の店舗へ進みます…`);}else{failed++;setMessage(`${STORE_NAMES[ids[i]]||ids[i]} は失敗/タイムアウト。次の店舗へ進みます… (${e.name==='AbortError'?'最大300秒タイムアウト':e.message})`);}}finally{currentAbortRef.current=null;clearInterval(timer);clearInterval(progressPoll);}
      }
      const finalRes=await fetch('/api/update',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'finalize',batchId,snapshot})});const finalJson=await finalRes.json();if(!finalRes.ok)throw new Error(finalJson.error||'履歴保存に失敗しました');setData(finalJson);setMessage(`更新完了：${ids.length-failed-skipped}店舗成功 / ${skipped}店舗スキップ / ${failed}店舗失敗。履歴を保存しました`);
  ...
  return <main>
    <header className="topbar"><div><p className="eyebrow">TOKUSHIMA BABY SALE</p><div className="mainTitleRow"><h1>ベビー用品 チラシチェッカー</h1><span className="versionBadge">ver 2.14</span></div><p className="sub">指定された各社の公式URLだけを使用し、Shufoo!では検索しません。取得状況を「チラシを発見」「OCRを実行中」などリアルタイム表示します。最新性は掲載側の日付とチラシ内の日付の2段階で確認します。コストコオンラインは指定した2ページ内で、赤文字の「引き後」が確認できる商品を画像付きで表示します。ベビー服は対象外です。</p></div><div className="actions"><a className="ghostButton" href="/api/history.csv">CSV履歴</a><button className="updateButton" onClick={update} disabled={loading}>{loading?'解析中…':'↻ 最新情報に更新'}</button></div></header>
