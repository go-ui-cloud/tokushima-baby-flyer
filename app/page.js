  ...
  async function update(){
    ...
    try{let snapshot={...data,results:[...(data.results||[])]};let failed=0;
      for(let i=0;i<ids.length;i++){
        ...
        catch(e){failed++;setMessage(`${STORE_NAMES[ids[i]]||ids[i]} は失敗/タイムアウト。次の店舗へ進みます… (${e.name==='AbortError'?'最大300秒タイムアウト':e.message})`);}finally{clearInterval(timer);}
      }
