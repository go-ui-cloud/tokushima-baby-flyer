...
export async function findExpiredAppImageUrls(retentionDays=14){
  if(!hasBlobStorage())return {urls:[],enabled:false,cutoff:null};
  const days=Math.max(1,Math.floor(Number(retentionDays)||14));
  const cutoff=new Date(Date.now()-days*24*60*60*1000);
  const urls=[];
  for(const prefix of ['flyers/','manual/']){
    let cursor;
    do{
      const page=await list({prefix,cursor,limit:100});
      for(const blob of page.blobs||[]){
        const uploadedAt=new Date(blob.uploadedAt);
        if(blob.url&&!Number.isNaN(uploadedAt.getTime())&&uploadedAt<=cutoff)urls.push(blob.url);
      }
      cursor=page.hasMore?page.cursor:undefined;
    }while(cursor);
  }
  return {urls:[...new Set(urls)],enabled:true,cutoff:cutoff.toISOString()};
}

export async function deleteAppImageUrls(urls=[]){
