...
export async function getLatest(){
  ...
  if(!sql) return {updatedAt:null,results:STORES.map(blankResult),persistence:{database:false,blob:Boolean(process.env.BLOB_READ_WRITE_TOKEN)}};
  ...
  return {updatedAt:newest?.toISOString()||null,results,persistence:{database:true,blob:Boolean(process.env.BLOB_READ_WRITE_TOKEN)}};
}
...
export async function finalizeHistory(batchId,snapshot=null){
  ...
  const payload={...latest,updatedAt:now,persistence:{database:Boolean(sql),blob:Boolean(process.env.BLOB_READ_WRITE_TOKEN)}};
