...
export async function getActiveProgress(maxAgeSeconds=900){
  ...
  const rows=await sql`SELECT store_id,batch_id,phase,detail,extra,updated_at FROM baby_flyer_progress WHERE updated_at >= now() - (${maxAgeSeconds} * interval '1 second') AND phase NOT IN ('完了','失敗','スキップ') ORDER BY updated_at DESC LIMIT 1`;
