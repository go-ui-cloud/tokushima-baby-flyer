...
export async function getLatest(){
  ...
  const results=STORES.map(s=>{
    ...
    return {
      ...
      chain:s.chain,
      area:s.area,
      storeKeywords:s.storeKeywords,
      sourceUrl:row.result?.sourceUrl||s.sources[0]?.url||'不明'
    };
  });
...
async function ensureManualItemsTable(sql){
  ...
  await sql`ALTER TABLE baby_flyer_manual_items ADD COLUMN IF NOT EXISTS source_type text`;
  await sql`ALTER TABLE baby_flyer_manual_items ADD COLUMN IF NOT EXISTS image_source_url text`;
  await sql`UPDATE baby_flyer_manual_items SET source_type='その他' WHERE source_type IS NULL OR source_type=''`;
}

export async function getManualItems(){
  const sql=await init(); if(!sql)return [];
  await ensureManualItemsTable(sql);
  const rows=await sql`SELECT id,store_id,product,image_url,image_blob_url,image_source_url,price,start_date,end_date,category,source_type,created_at FROM baby_flyer_manual_items ORDER BY created_at DESC,id DESC`;
  return rows.map(r=>({id:String(r.id),storeId:r.store_id,product:r.product,imageUrl:r.image_url||null,imageBlobUrl:r.image_blob_url||null,imageSourceUrl:r.image_source_url||null,price:r.price,startDate:r.start_date||'不明',endDate:r.end_date||'不明',category:r.category,sourceType:r.source_type||'その他',manual:true,createdAt:new Date(r.created_at).toISOString()}));
}

export async function addManualItem(item){
  const sql=await init(); if(!sql)throw new Error('Neon/Postgresが設定されていません');
  await ensureManualItemsTable(sql);
  const rows=await sql`INSERT INTO baby_flyer_manual_items (store_id,product,image_url,image_blob_url,image_source_url,price,start_date,end_date,category,source_type)
    VALUES (${item.storeId},${item.product},${item.imageUrl||null},${item.imageBlobUrl||null},${item.imageSourceUrl||null},${item.price},${item.startDate||null},${item.endDate||null},${item.category},${item.sourceType}) RETURNING id`;
...
export async function clearAllDatabaseData(){
  ...
  await ensureManualItemsTable(sql);
  await sql`CREATE TABLE IF NOT EXISTS baby_flyer_progress (store_id text PRIMARY KEY,batch_id text,phase text NOT NULL,detail text,extra jsonb,updated_at timestamptz NOT NULL DEFAULT now())`;
  await ensureSkipTable(sql);
  await sql`WITH
    deleted_manual AS (DELETE FROM baby_flyer_manual_items RETURNING 1),
    deleted_state AS (DELETE FROM baby_flyer_store_state RETURNING 1),
    deleted_history AS (DELETE FROM baby_flyer_update_history RETURNING 1),
    deleted_progress AS (DELETE FROM baby_flyer_progress RETURNING 1)
    DELETE FROM baby_flyer_skip_request`;
  return true;
}
