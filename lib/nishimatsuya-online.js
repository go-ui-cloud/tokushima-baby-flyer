...
function parsePage(html,url){
  ...
  $('.js_np-card-item').each((_,node)=>{
    ...
    items.push({category:nishimatsuyaOnlineCategory(url,name),product:name,price:`${Number(digits.replaceAll(',','')).toLocaleString('ja-JP')}円`,startDate:'不明',endDate:'不明',sourceUrl:url,sourceUrls:[url],flyerUrl:href,confidence:'西松屋公式・赤文字価格',notes:appeal,discountAfter:appeal||'赤文字価格',imageUrl:rawImage?abs(rawImage,url):null});
  });
  $('a[href]').each((_,node)=>{
      ...
      try{
        ...
        for(const offset of [0,60,120,180]){
          ...
          for(const row of rows){
            ...
            const item={category:nishimatsuyaOnlineCategory(source.url,row.name),product:row.name,price:`${Number(digits.replaceAll(',','')).toLocaleString('ja-JP')}円`,startDate:'不明',endDate:'不明',sourceUrl:source.url,sourceUrls:[source.url],flyerUrl:row.href,confidence:'西松屋公式・赤文字価格',notes:row.appeal,discountAfter:row.appeal||'赤文字価格',imageUrl:row.imageUrl?abs(row.imageUrl,source.url):null};
