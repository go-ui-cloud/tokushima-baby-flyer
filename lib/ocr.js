...
async function ocrBuffer(buffer,parameters=null){
  const worker=await getWorker();
  // 領域OCRの設定を次の通常OCRへ持ち越さない。
  await worker.setParameters(parameters||{tessedit_pageseg_mode:'3'});
  const {data}=await withTimeout(worker.recognize(buffer),OCR_PAGE_TIMEOUT_MS,'画像OCR');
  return data.text||'';
}
async function ocrBufferDetailed(buffer,parameters=null){
  const worker=await getWorker();
  await worker.setParameters(parameters||{tessedit_pageseg_mode:'3'});
  const {data}=await withTimeout(worker.recognize(buffer,{}, {text:true,blocks:true}),OCR_PAGE_TIMEOUT_MS,'画像OCR');
  const lines=[];
  for(const block of data.blocks||[])for(const paragraph of block.paragraphs||[])for(const line of paragraph.lines||[]){
    const words=line.words||[];
    const confidence=words.length?words.reduce((n,w)=>n+(Number(w.confidence)||0),0)/words.length:0;
    lines.push({text:String(line.text||'').trim(),bbox:line.bbox||null,height:line.bbox?line.bbox.y1-line.bbox.y0:0,confidence});
  }
...
export async function ocrAssetRegion(asset,region={}){
  try{
    ...
    if(region.preprocess==='threshold'){
      const pixels=ctx.getImageData(0,0,canvas.width,canvas.height);
      const threshold=Number(region.threshold)||225;
    ...
    if(region.preprocess==='red'){
      const pixels=ctx.getImageData(0,0,canvas.width,canvas.height);
      for(let i=0;i<pixels.data.length;i+=4){
        const r=pixels.data[i],g=pixels.data[i+1],b=pixels.data[i+2];
        const isRed=r>=110&&r>=g*1.28&&r>=b*1.12;
        const value=isRed?0:255;
        pixels.data[i]=value;pixels.data[i+1]=value;pixels.data[i+2]=value;
      }
      ctx.putImageData(pixels,0,0);
    }
    if(region.preprocess==='grayscale'){
      const pixels=ctx.getImageData(0,0,canvas.width,canvas.height);
      const contrast=Number(region.contrast)||1.5;
      for(let i=0;i<pixels.data.length;i+=4){
        const luminance=0.299*pixels.data[i]+0.587*pixels.data[i+1]+0.114*pixels.data[i+2];
    ...
    const parameters=region.pageSegMode?{tessedit_pageseg_mode:String(region.pageSegMode),preserve_interword_spaces:'1'}:null;
    if(region.detailed){
      const detailed=await ocrBufferDetailed(canvas.toBuffer('image/png'),parameters);
      return {...detailed,error:null};
    }
    const text=await ocrBuffer(canvas.toBuffer('image/png'),parameters);
