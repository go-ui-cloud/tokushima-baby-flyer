...
async function openNishimatsuyaTargetAndCapture(browser,target,runDir,index,progress){
  ...
  try{
    ...
    await progress('チラシページを開いています','西松屋「衛生・調乳グッズ」のページを開いています',{url:target.href});
    await page.goto(target.href,{waitUntil:'domcontentloaded',timeout:22000});
    await waitForVisualReady(page,{minWait:6500,maxWait:18000});
    await progress('チラシを発見','「衛生・調乳グッズ」の掲載商品を上から下まで読み取ります',{url:page.url()});
    const shots=await captureViewer(page,runDir,'nishimatsuya-hygiene-formula',index,{preferFull:false,maxSegments:18});
    return shots.map(s=>({...s,allItemsPage:true,sourceGroup:'衛生・調乳グッズ'}));
  }finally{if(page)await page.close().catch(()=>{});}
}
