...
async function ensureJapaneseFonts(page){
  // Runtime web-font fallback. If network/CSP blocks it, DOM extraction below
  // still preserves Japanese product names and prices.
  try{
...
async function collectDomProducts(page,{limit=240}={}){
  try{
    return await page.evaluate((limit)=>{
      const clean=v=>(v||'').normalize?.('NFKC')?.replace(/\s+/g,' ').trim()
        || String(v||'').replace(/\s+/g,' ').trim();
      const priceRe=/(?:税込|税抜|本体価格)?\s*(?:[¥￥]\s*)?\d{1,3}(?:,\d{3})*(?:\.\d+)?\s*円?|[¥￥]\s*\d{1,3}(?:,\d{3})*/;
      const selectors=[
        '[class*="product" i]','[class*="item" i]','[class*="goods" i]',
        '[data-product]','[data-item]','article','li'
      ];
      ...
      for(const sel of selectors){
        for(const el of document.querySelectorAll(sel)){
          ...
          if(r.width<100||r.height<60||r.height>1800)continue;
          const text=clean(el.innerText||el.textContent||'');
          if(text.length<3||text.length>1000||!priceRe.test(text))continue;

          const link=el.matches?.('a[href]')?el:el.querySelector?.('a[href]');
          const img=el.querySelector?.('img');
          const nameNode=el.querySelector?.(
            '[class*="name" i],[class*="title" i],[data-product-name],h1,h2,h3,h4,strong'
          );
          let name=clean(nameNode?.innerText||nameNode?.textContent||'');
          if(!name){
            const lines=(el.innerText||el.textContent||'').split(/\n+/).map(clean).filter(Boolean);
            name=lines.find(x=>!priceRe.test(x)&&x.length>=2&&x.length<=120)||'';
          }
          const pm=text.match(priceRe);
          const price=pm?clean(pm[0]):'';
          const href=link?.href||'';
          const image=img?.currentSrc||img?.src||img?.getAttribute?.('data-src')||'';
          if(!name||!price)continue;
          const key=clean(href||`${name}|${price}`).toLowerCase();
          if(seen.has(key))continue;
          seen.add(key);
          out.push({name,price,text,href,image});
        }
      }
...
async function hydrateLazyMedia(page,{maxSteps=36,stepRatio=0.72,settleMs=450}={}){
  // Common to every store: force lazy-loaded images toward eager loading, then
  // actually scroll through the page so IntersectionObserver-based sites render
  // the same content a person would see.
  try{
    await page.evaluate(()=>{
      for(const img of document.images){
        try{img.loading='eager';img.decoding='async';}catch{}
        ...
        if((!current || /^data:image\/(?:gif|png);base64/i.test(current)) && candidates[0]){
          try{img.src=candidates[0];}catch{}
        }
        const srcset=img.getAttribute('data-srcset')||img.dataset?.srcset;
        if(srcset && !img.getAttribute('srcset'))try{img.setAttribute('srcset',srcset);}catch{}
      }
    });
  }catch{}

  let dims={height:0,vh:1000};
  try{
    dims=await page.evaluate(()=>({
      height:Math.max(document.documentElement.scrollHeight,document.body?.scrollHeight||0),
      vh:Math.max(650,window.innerHeight||900)
    }));
  }catch{}

  const step=Math.max(480,Math.floor((dims.vh||900)*stepRatio));
  let y=0;
  for(let i=0;i<maxSteps;i++){
    ...
    // Wait briefly for media currently inside/near the viewport.
    try{
      await page.evaluate(async()=>{
        const sleepInPage=ms=>new Promise(r=>setTimeout(r,ms));
        const visible=()=>[...document.images].filter(img=>{
          const r=img.getBoundingClientRect();
          return r.bottom>=-250 && r.top<=(window.innerHeight||900)+350 && r.width>=100 && r.height>=80;
        });
        const started=Date.now();
        while(Date.now()-started<1800){
          const imgs=visible();
          if(!imgs.length || imgs.every(img=>img.complete && img.naturalWidth>30 && img.naturalHeight>30))break;
          await sleepInPage(180);
        }
      });
    }catch{}

    const maxY=Math.max(0,(dims.height||0)-(dims.vh||900));
    if(y>=maxY-20)break;
    y=Math.min(maxY,y+step);
  }

  // A second pass jumps directly to still-unloaded large images.
  try{
    const pending=await page.evaluate(()=>[...document.images].map((img,i)=>{
      const r=img.getBoundingClientRect();
      return {i,y:r.top+scrollY,w:r.width,h:r.height,loaded:img.complete&&img.naturalWidth>30&&img.naturalHeight>30};
    }).filter(x=>!x.loaded&&x.w>=120&&x.h>=80).slice(0,10));
    for(const item of pending){
      await page.evaluate(v=>window.scrollTo(0,Math.max(0,v-250)),item.y).catch(()=>{});
      await sleep(700);
    }
  }catch{}
  ...
  try{
    return await page.evaluate(()=>{
      ...
      const pending=imgs.filter(img=>!img.complete||img.naturalWidth<30||img.naturalHeight<30).length;
      return {images:imgs.length,pending};
    });
  }catch{return {images:0,pending:0};}
}
...
async function safeViewportScreenshot(page,file,{y=0,width=1365,height=1100}={}){
  try{
    ...
    await page.evaluate(v=>window.scrollTo(0,v),Math.max(0,y));await sleep(750);
    try{
      await page.evaluate(async()=>{
        const wait=ms=>new Promise(r=>setTimeout(r,ms));
        const started=Date.now();
        while(Date.now()-started<1600){
          const imgs=[...document.images].filter(img=>{const r=img.getBoundingClientRect();return r.bottom>=0&&r.top<=(window.innerHeight||1000)&&r.width>=100&&r.height>=80;});
          if(!imgs.length||imgs.every(img=>img.complete&&img.naturalWidth>30&&img.naturalHeight>30))break;
          await wait(160);
        }
      });
...
async function captureViewer(page,runDir,tag,index=0,{maxSegments=7,preferFull=false}={}){
  ...
  await ensureJapaneseFonts(page);
  const lazyState=await hydrateLazyMedia(page,{maxSteps:maxSegments>10?42:34,stepRatio:0.72,settleMs:500});
  if(lazyState?.pending>0){
    // Give stubborn image viewers one more short settling window before capture.
    await waitForVisualReady(page,{minWait:1800,maxWait:5000});
  }
  const domProducts=await collectDomProducts(page,{limit:240});
  const attachDomProducts=arr=>arr.map((o,i)=>i===0?{...o,domProducts}:o);
  ...
  if(outputs.length&&!preferFull)return attachDomProducts(outputs);
  ...
  return attachDomProducts(outputs);
}
...
async function openTargetAndCaptureByRealClick(browser,sourceUrl,target,runDir,tag,index,progress,{preferFull=false,maxNext=0,nextPatterns=[],turnPatterns=[]}={}){
  let page=null,popup=null;
  try{
    page=await browser.newPage();await preparePage(page);
    await page.goto(sourceUrl,{waitUntil:'domcontentloaded',timeout:22000});
    await waitForVisualReady(page,{minWait:5000,maxWait:15000});
    await progress('チラシをクリック中','元ページ上のチラシを実際にクリックします',{url:sourceUrl,target:target.text});

    const popupPromise=new Promise(resolve=>{
      const timer=setTimeout(()=>resolve(null),14000);
      page.once('popup',p=>{clearTimeout(timer);resolve(p);});
    });
    ...
    const clicked=await page.evaluate(({text,top,href})=>{
      ...
      const t=norm(text).slice(0,100);
      let e=els.find(x=>{
        const root=x.tagName==='IMG'?(x.closest('a,button,[role="button"]')||x):x;
        const tx=norm(`${root.innerText||''} ${x.alt||''} ${root.getAttribute?.('aria-label')||''}`);
        const hr=root.href||root.closest?.('a[href]')?.href||'';
        return (t&&tx.includes(t))||(href&&hr===href)||Math.abs((root.getBoundingClientRect().top+scrollY)-top)<14;
      });
      if(!e)return false;
      e=e.tagName==='IMG'?(e.closest('a,button,[role="button"]')||e):e;
    ...
    await navPromise;
    let active=popup||page;
    if(popup)await preparePage(popup).catch(()=>{});

    await waitForVisualReady(active,{minWait:7000,maxWait:22000});
    await ensureJapaneseFonts(active);
    await hydrateLazyMedia(active,{maxSteps:48,stepRatio:0.66,settleMs:650});

    const blocked=await active.evaluate(()=>document.body?.innerText?.slice(0,8000)||'').catch(()=>'');
    if(/Access Denied|permission to access|Forbidden|Request blocked|アクセスが拒否/i.test(blocked)){
      await progress('アクセス拒否を検出','クリック先がアクセス拒否のため保存しません');
      return [];
    }

    await progress('チラシを発見','バースデイのチラシ表示を確認しました。PDFではなく画像としてOCRします',{url:active.url()});
    let all=[];
    all.push(...await captureViewer(active,runDir,tag,index,{preferFull:false,maxSegments:16}));

    const repeat=[...nextPatterns,...turnPatterns];
    for(let n=0;n<maxNext;n++){
      const targets=await collectClickTargets(active,repeat);
      if(!targets.length)break;
      const t=targets[0];
      const before=active.url();
      try{
        if(t.href&&t.href!==before){
          await active.goto(t.href,{waitUntil:'domcontentloaded',timeout:16000});
        }else{
          await active.evaluate(({text,top})=>{
            const els=[...document.querySelectorAll('a,button,[role="button"],img')];
            const e=els.find(x=>((x.innerText||x.alt||'').replace(/\s+/g,' ').trim().includes((text||'').slice(0,80)))||Math.abs((x.getBoundingClientRect().top+scrollY)-top)<12);
            (e?.closest('a,button,[role="button"]')||e)?.click();
          },{text:t.text,top:t.top});
        }
        await waitForVisualReady(active,{minWait:5000,maxWait:14000});
        await hydrateLazyMedia(active,{maxSteps:40,stepRatio:0.68,settleMs:600});
        await progress('次のチラシを発見',`${n+2}枚目も画像としてOCR対象に追加します`,{url:active.url()});
        all.push(...await captureViewer(active,runDir,`${tag}-next`,n,{preferFull:false,maxSegments:16}));
      }catch{
        break;
      }
    }
...
async function captureBirthdayProductBlocks(page,runDir,flyerNo,progress){
  await ensureJapaneseFonts(page);
  await hydrateLazyMedia(page,{maxSteps:64,stepRatio:0.62,settleMs:650});
  await waitForVisualReady(page,{minWait:2500,maxWait:9000});

  // 「拡大して見る」の先に表示されたチラシを、価格が書かれた商品単位の
  // DOMブロック/画像ブロックとして保存する。1ブロック=1 OCR対象。
  const candidates=await page.evaluate(()=>{
    const norm=s=>(s||'').normalize('NFKC').replace(/\s+/g,' ').trim();
    const price=/(?:[¥￥]\s*)?\d{2,3}(?:,\d{3})+\s*円?|(?:[¥￥]\s*)?\d{3,6}\s*円|税込\s*\d{3,6}/;
    const bad=/HOME|店舗検索|オンラインストア|メニュー|次へ|前へ|閉じる|拡大|チラシ一覧/i;
    const selectors=['article','li','[class*="item" i]','[class*="product" i]','[class*="goods" i]','img'];
    const raw=[];
    for(const sel of selectors){
      for(const el of document.querySelectorAll(sel)){
        const r=el.getBoundingClientRect();
        if(r.width<180||r.height<120||r.height>1800)continue;
        const img=el.tagName==='IMG'?el:el.querySelector('img');
        const text=norm(`${el.innerText||''} ${el.textContent||''} ${img?.alt||''}`);
        const src=img?.currentSrc||img?.src||'';
        if(bad.test(text)||(!price.test(text)&&!src))continue;
        raw.push({sel,top:r.top+scrollY,left:r.left+scrollX,width:r.width,height:r.height,text:text.slice(0,1200),src});
      }
    }
    raw.sort((a,b)=>a.top-b.top||a.left-b.left||(a.width*a.height-b.width*b.height));
    // Near-identical nested candidates are collapsed.
    const out=[];
    for(const c of raw){
      if(out.some(x=>Math.abs(x.top-c.top)<8&&Math.abs(x.left-c.left)<8&&Math.abs(x.width-c.width)<12&&Math.abs(x.height-c.height)<12))continue;
      out.push(c);
    }
  ...
  const shots=[];
  for(let i=0;i<candidates.length&&shots.length<100;i++){
    const c=candidates[i];
    try{
      await page.evaluate(({top})=>scrollTo(0,Math.max(0,top-220)),c);
      await sleep(250);
      const file=path.join(runDir,`birthday-${flyerNo}-block-${String(shots.length+1).padStart(2,'0')}.png`);
      const shot=await safeViewportScreenshot(page,file,{
        x:Math.max(0,Math.floor(c.left)),
        y:Math.max(0,Math.floor(c.top)),
        width:Math.max(180,Math.floor(c.width)),
        height:Math.max(120,Math.floor(c.height))
      });
      if(!shot||shot.size<7000)continue;
      shots.push({...shot,tag:`birthday-${flyerNo}-block-${shots.length+1}`,sourceGroup:`バースデイ チラシ${flyerNo} 商品ブロック ${shots.length+1}`,birthdayBlock:true,allItemsPage:false});
    }catch{}
  }
  await progress('商品ブロック取得完了',`バースデイ チラシ${flyerNo} を下まで確認し、商品ブロック画像を ${shots.length} 件取得しました`,{count:shots.length,url:page.url()});
  return shots;
}

async function openBirthdayTargetAndCapture(browser,sourceUrl,target,runDir,index,progress){
  let page=null,popup=null;
  try{
    page=await browser.newPage(); await preparePage(page);
    await page.goto(sourceUrl,{waitUntil:'domcontentloaded',timeout:22000});
    await waitForVisualReady(page,{minWait:5000,maxWait:15000});
    await progress('拡大ボタンをクリック中',`バースデイの「拡大して見る」${index+1}件目を実際にクリックします`,{url:sourceUrl,target:target.text});
    ...
    if(popup)await preparePage(popup).catch(()=>{});
    await waitForVisualReady(active,{minWait:7000,maxWait:22000});
    await hydrateLazyMedia(active,{maxSteps:64,stepRatio:0.62,settleMs:650});
    await progress('拡大チラシを確認',`チラシ${index+1}のページを下まで読み込み、商品ごとのブロックに分割します`,{url:active.url()});

    let shots=await captureBirthdayProductBlocks(active,runDir,index+1,progress);
    if(!shots.length){
      await progress('商品ブロック検出不足','個別商品ブロックを検出できなかったため、チラシを縦方向に分割してOCRします');
      shots=await captureViewer(active,runDir,`birthday-${index+1}`,index,{preferFull:false,maxSegments:20});
      shots=shots.map((x,i)=>({...x,birthdayBlock:true,sourceGroup:`バースデイ チラシ${index+1} 分割 ${i+1}`}));
    }
    return shots;
  }finally{
    if(popup)await popup.close().catch(()=>{});
...
function assetsFromShots(shots,sourceUrl,sourceDateCheck,label){return shots.map((s,i)=>({url:`screenshot://${hash(`${sourceUrl}-${s.tag}-${i}`)}`,referer:sourceUrl,file:s.file,mime:'image/png',size:s.size,score:100,width:s.width,height:s.height,context:'store-specific-click-flow',captureMethod:'screenshot',provider:'official',sourceLabel:label,sourceDateCheck,domProducts:s.domProducts||[]}));}


async function captureNishimatsuyaProductBlocks(page,runDir,pageNo,progress){
  await ensureJapaneseFonts(page);
  await hydrateLazyMedia(page,{maxSteps:70,stepRatio:0.62,settleMs:650});
  await waitForVisualReady(page,{minWait:2500,maxWait:8000});

  // 西松屋の /04/ /05/ /06/ は商品ブロック画像が縦に並ぶページ。
  // ページ全体を巨大スクショにせず、表示中の大きな商品画像を1ブロックずつ保存する。
  const candidates=await page.evaluate(()=>{
    const norm=s=>(s||'').normalize('NFKC').replace(/\s+/g,' ').trim();
    const bad=/HOME|掲載商品一覧|オンラインストアはこちら|おすすめコンテンツ|店舗ごとのSALE|折り込みチラシタイプ/i;
    const imgs=[...document.images];
    return imgs.map((img,i)=>{
      const r=img.getBoundingClientRect();
      const alt=norm(img.alt||'');
      const parent=img.closest('a,li,article,section,div');
      const ctx=norm(`${alt} ${parent?.innerText||''} ${parent?.getAttribute?.('class')||''}`);
      return {
        i,top:r.top+scrollY,left:r.left+scrollX,width:r.width,height:r.height,
        nw:img.naturalWidth||0,nh:img.naturalHeight||0,alt,ctx:ctx.slice(0,500),
        src:img.currentSrc||img.src||''
      };
    }).filter(x=>{
      if(!x.src||bad.test(`${x.alt} ${x.ctx}`))return false;
      // Product artwork on these pages is large enough to OCR independently.
      return (x.width>=240&&x.height>=180)||(x.nw>=400&&x.nh>=300);
  ...
  for(const c of candidates){
    ...
    try{
      ...
      if(!box||box.width<180||box.height<130)continue;
      const file=path.join(runDir,`nishimatsuya-${pageNo}-block-${String(shots.length+1).padStart(2,'0')}.png`);
      await el.screenshot({path:file,type:'png'});
      const st=await fs.stat(file);
      if(st.size<8000)continue;
      shots.push({
        file,size:st.size,width:Math.round(box.width),height:Math.round(box.height),
        tag:`nishimatsuya-${pageNo}-block-${shots.length+1}`,
        sourceGroup:`西松屋 /${pageNo}/ 商品ブロック ${shots.length+1}`,
        allItemsPage:true,
        nishimatsuyaBlock:true,
        blockIndex:shots.length+1
      });
    }catch{}
  }
  await progress('商品ブロック取得完了',`西松屋 /${pageNo}/ を下まで読み込み、商品ブロック画像を ${shots.length} 件取得しました`,{pageNo,count:shots.length,url:page.url()});
  return shots;
}

async function openNishimatsuyaTargetAndCapture(browser,target,runDir,index,progress){
  let page=null;
  try{
    page=await browser.newPage();await preparePage(page);
    if(!target.href||!/^https?:/i.test(target.href))return[];
    const m=target.href.match(/\/(04|05|06)\/?(?:[?#].*)?$/);
    const pageNo=m?.[1]||String(index+4).padStart(2,'0');
    await progress('チラシページを開いています',`西松屋 /${pageNo}/ を開いています`,{url:target.href});
    await page.goto(target.href,{waitUntil:'domcontentloaded',timeout:22000});
    await waitForVisualReady(page,{minWait:7000,maxWait:20000});
    await progress('ページ最下部まで読込中',`西松屋 /${pageNo}/ を上から下までスクロールして遅延画像を読み込みます`,{url:page.url()});

    let shots=await captureNishimatsuyaProductBlocks(page,runDir,pageNo,progress);
    if(!shots.length){
      await progress('商品ブロック検出不足',`/${pageNo}/ の個別商品画像を検出できなかったため、縦方向の分割スクリーンショットへ切り替えます`);
      shots=await captureViewer(page,runDir,`nishimatsuya-${pageNo}`,index,{preferFull:false,maxSegments:24});
      shots=shots.map((x,i)=>({...x,allItemsPage:true,sourceGroup:`西松屋 /${pageNo}/ 分割 ${i+1}`,nishimatsuyaBlock:false}));
    }
    return shots;
  }finally{if(page)await page.close().catch(()=>{});}
}
...
async function collectAkachanDealTargetsByText(page,progress){
  ...
  const ak=targets.filter(x=>x.kind==='アカトク').slice(0,2);
  const diaper=targets.filter(x=>x.kind==='紙おむつセール').slice(0,2);
  const out=[...diaper,...ak];
  for(const t of out){
    await progress('対象バナーを発見',`${t.text} をページ内の表示文字から確認しました`,{target:t.text,url:t.href,occurrence:t.occurrence});
  }
  if(ak.length<2)await progress('対象バナー確認',`アカトクは ${ak.length}/2 件確認できました。見つからない分は推測してクリックしません`,{found:ak.length,expected:2});
  if(diaper.length<2)await progress('対象バナー確認',`紙おむつセールは ${diaper.length}/2 件確認できました。見つからない分は推測してクリックしません`,{found:diaper.length,expected:2});
...
async function storeSpecificFlow(store,browser,runDir,progress){
  ...
  try{
    ...
    if(store.id==='nishimatsuya'){
      await progress('今週のチラシURLを確認中','西松屋のデジタルチラシ入口から、リダイレクト後の今週URLを確認しています',{url:page.url()});
      // Redirect can finish a little after DOMContentLoaded. Wait until the URL
      // actually contains the current period path rather than assuming immediately.
      let redirected=page.url();
      const waitStarted=Date.now();
      while(Date.now()-waitStarted<15000 && !/\/digital-chirashi\/period-[^/]+\/?(?:[?#].*)?$/i.test(redirected)){
        await sleep(500);
        redirected=page.url();
      }
      let hygieneUrl=''; // current period /04/ URL seed
      try{
        const u=new URL(redirected);
        let pathname=u.pathname.replace(/\/+$/,'');
        const m=pathname.match(/^(.*\/digital-chirashi\/period-[^/]+)$/i);
        if(m){
          u.pathname=`${m[1]}/04/`; u.search=''; u.hash='';
          hygieneUrl=u.toString();
        }
      }catch{}
      if(hygieneUrl){
        const base=hygieneUrl.replace(/\/04\/$/,'');
        targets=[
          {href:`${base}/04/`,text:'西松屋 今週ページ /04/',top:0},
          {href:`${base}/05/`,text:'西松屋 今週ページ /05/',top:0},
          {href:`${base}/06/`,text:'西松屋 今週ページ /06/',top:0},
        ];
        await progress('対象URLを確定','今週の period URL を確認後、/04/・/05/・/06/ の3ページだけを直接読み込みます。衛生・調乳グッズのリンク探索は行いません',{redirectedUrl:redirected,urls:targets.map(x=>x.href)});
      }else{
        targets=[];
        await progress('対象URL確認不足','15秒待っても今週の period URL を確認できなかったため、/04/・/05/・/06/ を推測して作成しません',{url:redirected});
      }
      opts={maxNext:0,nextPatterns:[],turnPatterns:[],preferFull:false};
    }else if(store.id==='birthday-aizumi'){
      await progress('チラシ表示を確認中','バースデイ藍住店の公式店舗ページで「拡大して見る」をクリックし、その先を商品ブロック単位でOCRします');
    ...
    for(let i=0;i<targets.length;i++){
      ...
      const shots=store.id==='akachan-aizumi'?await openAkachanTargetAndCapture(browser,source.url,targets[i],runDir,i,progress):store.id==='nishimatsuya'?await openNishimatsuyaTargetAndCapture(browser,targets[i],runDir,i,progress):store.id==='birthday-aizumi'?await openBirthdayTargetAndCapture(browser,source.url,targets[i],runDir,i,progress):store.id==='lady'?await openTargetAndCaptureByRealClick(browser,source.url,targets[i],runDir,store.id,i,progress,opts):await openTargetAndCapture(browser,page,targets[i],runDir,store.id,i,progress,opts);
      if(shots.length){const shotAssets=assetsFromShots(shots,targets[i].href||source.url,sourceDateCheck,source.label).map((a,si)=>({...a,sourceGroup:shots[si]?.sourceGroup||targets[i].text,allowedCategories:shots[si]?.allowedCategories||null,allItemsPage:shots[si]?.allItemsPage||false,birthdayBlock:shots[si]?.birthdayBlock||false,nishimatsuyaBlock:shots[si]?.nishimatsuyaBlock||false}));assets.push(...shotAssets);await progress('スクリーンショット取得完了',`${targets[i].text}: ${shots.length} 枚をOCR対象に追加しました`,{count:shots.length,target:targets[i].text});}
      if(assets.length>=(store.id==='nishimatsuya'?240:store.id==='akachan-aizumi'?80:store.id==='birthday-aizumi'?120:12))break;
    }
