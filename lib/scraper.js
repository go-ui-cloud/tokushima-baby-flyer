...
async function collectAkachanDealTargetsByOcr(page,runDir,progress){
  await progress('セール画像を確認中','セール・チラシ情報のバナー画像をOCRして「アカトク」「紙おむつSALE」を探しています');
  await page.evaluate(()=>document.querySelector('#deals')?.scrollIntoView({block:'start'})).catch(()=>{});
  await sleep(1800);
  const candidates=await page.$$('#deals img, [id*="deal"] img, section img, article img');
  const found=[];let checked=0;
  for(const img of candidates.slice(0,30)){
    try{
      const box=await img.boundingBox();if(!box||box.width<180||box.height<70)continue;
      const meta=await img.evaluate(el=>{const a=el.closest('a[href]');return{src:el.currentSrc||el.src||'',alt:el.alt||'',href:a?.href||'',context:(a?.parentElement?.innerText||el.parentElement?.innerText||'').replace(/\s+/g,' ').trim().slice(0,500),top:el.getBoundingClientRect().top+scrollY};});
      const file=path.join(runDir,`akachan-deal-banner-${checked++}.png`);
      await img.screenshot({path:file,type:'png'});
      const ocr=await ocrAsset({file});
      const text=`${meta.alt} ${meta.context} ${ocr.text||''}`.replace(/\s+/g,' ').trim();
      const kind=/紙\s*[おオ]?むつ|紙オムツ|紙おむつ|diaper/i.test(text)?'紙おむつSALE':/アカ\s*トク/i.test(text)?'アカトク':null;
      if(kind){found.push({href:meta.href,text:kind,context:text.slice(0,1000),top:meta.top,width:box.width,height:box.height});await progress('対象バナーを発見',`${kind} を画像OCRで確認しました`,{target:kind,url:meta.href});}
      if(found.some(x=>x.text==='アカトク')&&found.some(x=>x.text==='紙おむつSALE'))break;
    }catch{}
  }
  const uniq=[];const seen=new Set();for(const x of found){if(seen.has(x.text))continue;seen.add(x.text);uniq.push(x);}return uniq;
}
...
async function storeSpecificFlow(store,browser,runDir,progress){
  ...
  try{
    ...
    if(store.id==='nishimatsuya'){
      ...
      targets=await collectAkachanDealTargetsByOcr(page,runDir,progress);
      if(!targets.length)targets=await collectClickTargets(page,[/アカトク/i,/紙おむつ.*(?:SALE|セール)/i,/紙オムツ.*(?:SALE|セール)/i],{scopeWords:[/セール.*チラシ情報/i,/SALE/i,/セール/i]});
    ...
    for(let i=0;i<targets.length;i++){
      ...
      const shots=(store.id==='birthday-aizumi'||store.id==='lady')?await openTargetAndCaptureByRealClick(browser,source.url,targets[i],runDir,store.id,i,progress,opts):await openTargetAndCapture(browser,page,targets[i],runDir,store.id,i,progress,opts);
