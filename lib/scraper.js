...
async function storeSpecificFlow(store,browser,runDir,progress){
  ...
  try{
    ...
    if(store.id==='nishimatsuya'){
      ...
      await progress('チラシ表示を確認中','バースデイはPDFを直接解析せず、「拡大して見る」の先をブラウザ表示してスクリーンショットOCRします');
      targets=await collectClickTargets(page,[/拡大して見る/i],{scopeWords:[/チラシ/i,/売出し期間/i]});
      opts={maxNext:2,nextPatterns:[/次へ/i,/次のチラシ/i,/裏面/i,/2\s*\/\s*2/i],turnPatterns:[],preferFull:false};
    ...
    for(let i=0;i<targets.length;i++){
      ...
      const shots=store.id==='akachan-aizumi'?await openAkachanTargetAndCapture(browser,source.url,targets[i],runDir,i,progress):store.id==='nishimatsuya'?await openNishimatsuyaTargetAndCapture(browser,targets[i],runDir,i,progress):(store.id==='birthday-aizumi'||store.id==='lady')?await openTargetAndCaptureByRealClick(browser,source.url,targets[i],runDir,store.id,i,progress,opts):await openTargetAndCapture(browser,page,targets[i],runDir,store.id,i,progress,opts);
      if(shots.length){const shotAssets=assetsFromShots(shots,targets[i].href||source.url,sourceDateCheck,source.label).map((a,si)=>({...a,sourceGroup:shots[si]?.sourceGroup||targets[i].text,allowedCategories:shots[si]?.allowedCategories||null,allItemsPage:shots[si]?.allItemsPage||false}));assets.push(...shotAssets);await progress('スクリーンショット取得完了',`${targets[i].text}: ${shots.length} 枚をOCR対象に追加しました`,{count:shots.length,target:targets[i].text});}
      if(assets.length>=(store.id==='nishimatsuya'?240:store.id==='akachan-aizumi'?80:12))break;
    }
    ...
    return{pages,assets:assets.slice(0,store.id==='nishimatsuya'?240:store.id==='akachan-aizumi'?80:12),selectedSource:source,attempts};
