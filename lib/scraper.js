...
async function storeSpecificFlow(store,browser,runDir,progress){
  ...
  try{
    ...
    if(store.id==='nishimatsuya'){
      await progress('対象カテゴリを確認中','西松屋デジタルチラシから「衛生・調乳グッズ」を探しています');
      targets=await collectClickTargets(page,[/衛生\s*[・･]?\s*調乳グッズ/i,/衛生.*調乳/i]);
      opts={maxNext:0,nextPatterns:[],turnPatterns:[],preferFull:false};
      if(targets.length)await progress('対象リンクを発見','「衛生・調乳グッズ」を確認しました。クリック先は掲載商品すべてを対象にします');
