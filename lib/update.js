...
import { scrapeUniqloOnline } from './uniqlo.js';
import { dedupeUniqlo } from './uniqlo-dedupe.js';
import { scrapeAkachanOnline } from './akachan-online.js';
import { scrapeNishimatsuyaOnline } from './nishimatsuya-online.js';
...
export async function updateStore(storeId,batchId=null,options={}){
  ...
  if(store.type==='nishimatsuya-online'){
    try{
      await progress('店舗ページ確認中','西松屋公式オンラインストアを確認しています');
      const out=await scrapeNishimatsuyaOnline(store,progress);
      const failed=out.pages.filter(x=>x.error),warnings=[];
      if(failed.length===out.pages.length)throw new Error(`指定ページを取得できませんでした: ${failed.map(x=>x.label).join('、')}`);
      if(failed.length)warnings.push(`取得できない情報元が${failed.length}件ありました: ${failed.map(x=>x.label).join('、')}`);
      if(!out.items.length)warnings.push('指定ページで赤文字価格の商品を確認できませんでした');
      await progress('完了',`${out.items.length}件の赤文字価格商品を確認しました`,{itemCount:out.items.length});
      const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:out.items,error:null,warnings,flyerFreshness:'オンライン商品ページ',acquisition:'nishimatsuya-official-html',browserWarning:null,sourceProvider:'西松屋公式',sourceAttempts:out.pages||[],durationMs:Date.now()-startedAt,extendedAnalysis:false,checkedAt:new Date().toISOString()};
  ...
  if(store.type==='akachan-online'){
    try{
      await progress('店舗ページ確認中','アカチャンホンポ公式オンラインショップを確認しています');
      const out=await scrapeAkachanOnline(store,progress);
      const failed=out.pages.filter(x=>x.error);
      if(failed.length===out.pages.length)throw new Error(`指定ページを取得できませんでした: ${failed.map(x=>x.label).join('、')}`);
      const warnings=[];
      if(failed.length)warnings.push(`取得できない情報元が${failed.length}件ありました: ${failed.map(x=>x.label).join('、')}`);
      if(!out.items.length)warnings.push('指定ページで赤文字価格の商品を確認できませんでした');
      await progress('完了',`${out.items.length}件の赤文字価格商品を確認しました`,{itemCount:out.items.length});
      const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:out.items,error:null,warnings,flyerFreshness:'オンライン商品ページ',acquisition:'akachan-official-html',browserWarning:null,sourceProvider:'アカチャンホンポ公式',sourceAttempts:out.pages||[],durationMs:Date.now()-startedAt,extendedAnalysis:false,checkedAt:new Date().toISOString()};
      await saveStoreResult(result);return result;
    }catch(e){
      await saveProgress(store.id,'エラー',e.message,batchId,{elapsedMs:Date.now()-startedAt}).catch(()=>{});
      return {id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:[],error:e.message,warnings:[],flyerFreshness:'オンライン商品ページ',durationMs:Date.now()-startedAt,extendedAnalysis:false,preservePrevious:true,checkedAt:new Date().toISOString()};
  ...
  if(store.type==='uniqlo-online'){
    try{
      await progress('店舗ページ確認中','UNIQLO公式のオンラインチラシを確認しています');
      const out=await scrapeUniqloOnline(store,progress);
      await progress('完了',`${out.items.length}件の対象商品を確認しました`,{itemCount:out.items.length});
      const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:dedupeUniqlo(out.items),error:null,warnings:out.items.length?[]:['赤文字・値下げ・期間限定価格の商品を確認できませんでした'],flyerFreshness:'オンラインチラシ',acquisition:'uniqlo-official-online',browserWarning:null,sourceProvider:'UNIQLO公式',sourceAttempts:out.pages||[],durationMs:Date.now()-startedAt,extendedAnalysis:false,checkedAt:new Date().toISOString()};
      await saveStoreResult(result);return result;
    }catch(e){
      ...
      await saveProgress(store.id,'エラー',e.message,batchId,{elapsedMs:Date.now()-startedAt}).catch(()=>{});
      return {id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:[],error:e.message,warnings:[],flyerFreshness:'オンラインチラシ',durationMs:Date.now()-startedAt,extendedAnalysis:false,preservePrevious:true,checkedAt:new Date().toISOString()};
    }
  }
