...
export async function updateStore(storeId,batchId=null){
  ...
  if(store.type==='costco-online'){
    try{
      ...
      await progress('完了',`${out.items.length}件の「○○円引き後」対象商品を確認しました`,{itemCount:out.items.length});
      const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,flyers:[],items:dedupe(out.items),error:null,warnings:out.items.length?[]:['「○○円引き後」が明記された対象カテゴリ商品を確認できませんでした'],flyerFreshness:'オンライン商品ページ',acquisition:'costco-official-html',browserWarning:null,sourceProvider:'コストコオンライン公式',sourceAttempts:out.pages||[],durationMs:Date.now()-startedAt,extendedAnalysis:false,checkedAt:new Date().toISOString()};
