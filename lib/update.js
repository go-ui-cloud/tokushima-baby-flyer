  ...
  try{
    ...
    for(let assetIndex=startAssetIndex;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(ocr.text&&!['stale','conflict'].includes(verification.status)){
        ...
        const fullItems=asset.allItemsPage?extractAllFlyerItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:asset.nishimatsuyaBlock?'西松屋・個別商品画像OCR':'対象ページ全商品OCR',fallbackCategory:store.id==='nishimatsuya'?'その他':undefined}):extractBabyItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:asset.captureMethod==='screenshot'?'チラシ全体OCR':asset.file.endsWith('.pdf')?'PDF/OCR抽出':'画像OCR抽出'});
        ...
        if(store.id==='nishimatsuya'&&asset.nishimatsuyaBlock){
          extracted=extracted.map(x=>({...x,imageUrl:x.imageUrl||displayUrl,sourceGroup:x.sourceGroup||asset.sourceGroup||null}));
          if(!extracted.length){
            extracted.push(nishimatsuyaFallbackFromOcr(ocr.text,{
    ...
    if(store.id==='akachan-aizumi'){
      ...
      items=dedupe(items).filter(x=>x.imageUrl).slice(0,240);
    }else{
      chosen=(verifiedCurrent.length?verifiedCurrent:verifiedRecent.length?verifiedRecent:flyerOnly.length?flyerOnly:sourceOnly.length?sourceOnly:flyers).slice(0,4);
