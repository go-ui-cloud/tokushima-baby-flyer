...
export async function updateStore(storeId,batchId=null,options={}){
  ...
  try{
    ...
    for(let assetIndex=startAssetIndex;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(ocr.text&&!['stale','conflict'].includes(verification.status)){
        ...
        if(store.id==='nishimatsuya'&&asset.nishimatsuyaBlock){
          ...
          const nameRegion=await ocrAssetRegion(asset,{x:0.00,y:0.58,width:0.68,height:0.42,scale:3.4,preprocess:'threshold',threshold:238,pageSegMode:6});
          const priceRegion=await ocrAssetRegion(asset,{x:0.54,y:0.54,width:0.46,height:0.46,scale:3.6,preprocess:'threshold',threshold:225,pageSegMode:6});
          // V2.31.3: 外部関数参照を介さず、この更新スコープ内で商品を生成する。
          // これによりデプロイ後の関数解決差異によるReferenceErrorを防ぐ。
          const taxIncludedPrice=nishimatsuyaTaxIncludedPrice(priceRegion.text);
          const productName=nishimatsuyaReadableName(nameRegion.text);
          const item=taxIncludedPrice?{
            category:nishimatsuyaClassify(`${productName}\n${nameRegion.text||''}\n${ocr.text||''}`),
            product:productName,
    ...
    if(store.id==='akachan-aizumi'){
      ...
      if(!items.length&&fallbackPreviousResult?.items?.length){
        items=fallbackPreviousResult.items;
        warnings.push('今回の税込価格OCRが0件だったため、前回正常取得した西松屋の商品表示を維持しました');
      }
    }else{
      chosen=(verifiedCurrent.length?verifiedCurrent:verifiedRecent.length?verifiedRecent:flyerOnly.length?flyerOnly:sourceOnly.length?sourceOnly:flyers).slice(0,4);
