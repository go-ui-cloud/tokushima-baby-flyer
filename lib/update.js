...
export async function updateStore(storeId,batchId=null){
  ...
  if(store.type==='costco-online'){
    try{
      ...
      if(e.code==='STORE_SKIPPED'){ const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,sourceUrls:store.sources.map(x=>({label:x.label,url:x.url})),flyers:[],items:[],error:null,warnings:['ユーザー操作でスキップしました'],flyerFreshness:'スキップ',durationMs:Date.now()-startedAt,extendedAnalysis:false,skipped:true,checkedAt:new Date().toISOString()}; await saveStoreResult(result).catch(()=>{}); await saveProgress(store.id,'スキップ','ユーザー操作でこの店舗をスキップしました',batchId,{elapsedMs:Date.now()-startedAt}).catch(()=>{}); return result; }
  ...
  try{
    ...
    for(let assetIndex=0;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(ocr.text&&!['stale','conflict'].includes(verification.status)){
        ...
        const fullItems=asset.allItemsPage?extractAllFlyerItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:'衛生・調乳グッズ全商品OCR'}):extractBabyItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:asset.captureMethod==='screenshot'?'チラシ全体OCR':asset.file.endsWith('.pdf')?'PDF/OCR抽出':'画像OCR抽出'});
  ...
  }catch(e){if(e.code==='STORE_SKIPPED'){const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0]?.url,flyers:[],items:[],error:null,warnings:['ユーザー操作でスキップしました'],flyerFreshness:'スキップ',durationMs:Date.now()-startedAt,extendedAnalysis:extended,skipped:true,checkedAt:new Date().toISOString()};await saveStoreResult(result).catch(()=>{});await saveProgress(store.id,'スキップ','ユーザー操作でこの店舗をスキップしました',batchId,{elapsedMs:Date.now()-startedAt}).catch(()=>{});return result;}await saveProgress(store.id,'エラー',e.message,batchId,{elapsedMs:Date.now()-startedAt}).catch(()=>{});const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:(scraped?.selectedSource?.url||store.sources[0]?.url),flyers:[],items:[],error:e.message,warnings,flyerFreshness:'不明',durationMs:Date.now()-startedAt,extendedAnalysis:extended,checkedAt:new Date().toISOString()};await saveStoreResult(result).catch(()=>{});return result;}
