...
export async function updateStore(storeId){
  ...
  try{
    ...
    const result={id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,sourceUrl:store.sources[0],flyers,items,error:null,acquisition:scraped.acquisition||'不明',browserWarning:scraped.browserError||null,checkedAt:new Date().toISOString()};
