...
function nishimatsuyaPrice(text){
  const s=String(text||'').normalize('NFKC');
  const candidates=[];
  const push=(raw,index,ctx='')=>{
    const value=Number(String(raw||'').replace(/[^\d]/g,''));
    if(!Number.isFinite(value)||value<100||value>200000)return;
    candidates.push({value,index,tax:/税込/.test(ctx),yen:/[¥￥]/.test(ctx)});
  };
  for(const m of s.matchAll(/[¥￥]\s*([0-9][0-9,]{2,7})/g)){
    push(m[1],m.index,s.slice(Math.max(0,m.index-18),Math.min(s.length,m.index+m[0].length+18)));
  }
  for(const m of s.matchAll(/([0-9][0-9,]{2,7})\s*円/g)){
    push(m[1],m.index,s.slice(Math.max(0,m.index-18),Math.min(s.length,m.index+m[0].length+18)));
  }
  if(!candidates.length)return '不明';
  // 税込価格を最優先。次に円記号付き。画像内の最初の明確な価格を採用。
  candidates.sort((a,b)=>(Number(b.tax)-Number(a.tax))||(Number(b.yen)-Number(a.yen))||(a.index-b.index));
  return `${candidates[0].value}円`;
}
function nishimatsuyaItemFromOcr(text,meta={}){
  const baby=extractBabyItems(text,{sourceUrl:meta.sourceUrl||'不明',flyerUrl:meta.flyerUrl||'不明',confidence:'西松屋・商品画像OCR'});
  const generic=extractAllFlyerItems(text,{sourceUrl:meta.sourceUrl||'不明',flyerUrl:meta.flyerUrl||'不明',confidence:'西松屋・商品画像OCR',fallbackCategory:'その他'});
  const base=baby[0]||generic[0]||null;
  const lines=String(text||'').replace(/\r/g,'').split('\n').map(x=>x.trim()).filter(Boolean);
  const noise=/^(税込|税抜|本体価格|各|sale|セール|ポイント|オンラインストアはこちら|商品番号|※|デジタルチラシ|home|掲載商品一覧)/i;
  const fallbackName=lines.find(x=>!noise.test(x)&&!/^[¥￥]?\s*\d[\d,]*\s*(?:円)?$/.test(x)&&x.length>=2&&x.length<=90)||'商品名不明';
  return {
    category:base?.category||'その他',
    product:base?.product&&base.product!=='商品名不明'?base.product:fallbackName,
    price:nishimatsuyaPrice(text),
    startDate:base?.startDate||'不明',
    endDate:'不明',
    sourceUrl:meta.sourceUrl||'不明',
    flyerUrl:meta.flyerUrl||'不明',
    confidence:'西松屋・1画像1商品OCR',
    notes:base?.notes||'西松屋の商品ブロック画像を1商品として表示',
    imageUrl:meta.imageUrl||null,
    sourceGroup:meta.sourceGroup||null
  };
}
    ...
    for(let assetIndex=startAssetIndex;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(ocr.text&&!['stale','conflict'].includes(verification.status)){
        ...
        if(store.id==='nishimatsuya'&&asset.nishimatsuyaBlock){
          // 西松屋はcapture段階ですでに1商品ブロック=1画像。
          // ここで再分割せず、OCR全文から1商品カードだけ作る。
          extracted=[nishimatsuyaItemFromOcr(ocr.text,{
          ...
          const fullItems=asset.allItemsPage?extractAllFlyerItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:'対象ページ全商品OCR'}):extractBabyItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:asset.captureMethod==='screenshot'?'チラシ全体OCR':asset.file.endsWith('.pdf')?'PDF/OCR抽出':'画像OCR抽出'});
