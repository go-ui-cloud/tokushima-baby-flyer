...
const BASE_LIMIT_MS=300000;
const EXTENDED_LIMIT_MS=600000;
...
export async function updateStore(storeId,batchId=null){
  ...
  try{
    ...
    if(looksHeavy(scraped.assets)){extended=true;deadline=startedAt+EXTENDED_LIMIT_MS;warnings.push('PDF/複数ページなど解析量が多いため、最大10分モードへ自動延長しました');await progress('10分モードへ延長','PDF/複数ページなど文字量が多いため最大10分まで延長します');}
    ...
    for(let assetIndex=0;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(Date.now()>deadline){warnings.push(extended?'10分上限に近づいたため、残りのOCRを省略しました':'通常5分上限に達したため、残りのOCRを省略しました');await progress('時間上限','残りのOCRを省略します');break;}
      ...
      if(!extended&&ocr.text?.length>=1800){extended=true;deadline=startedAt+EXTENDED_LIMIT_MS;warnings.push('文字量が多いため、最大10分モードへ自動延長しました');await progress('10分モードへ延長',`OCR文字量が多いため最大10分まで延長します`,{textLength:ocr.text.length});}
