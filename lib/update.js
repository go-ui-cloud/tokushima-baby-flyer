...
export async function updateStore(storeId){
  ...
  try{
    ...
    const result={
      ...
      sourceUrl:store.sources[0],flyers,items,error:null,warnings,
      acquisition:scraped.acquisition||'不明',browserWarning:scraped.browserError||null,
    ...
    const result={
      id:store.id,chain:store.chain,area:store.area,storeKeywords:store.storeKeywords,
      sourceUrl:store.sources[0],flyers:[],items:[],error:e.message,warnings,
      durationMs:Date.now()-startedAt,checkedAt:new Date().toISOString()
    };
