  ...
  try{
    ...
    for(let assetIndex=0;assetIndex<scraped.assets.length;assetIndex++){
      ...
      if(ocr.text&&!['stale','conflict'].includes(verification.status)){
        ...
        let extracted=extractBabyItems(ocr.text,{sourceUrl:asset.referer||(scraped.selectedSource?.url||store.sources[0]?.url),flyerUrl:displayUrl,confidence:asset.captureMethod==='screenshot'?'チラシ画面スクリーンショット/OCR':asset.file.endsWith('.pdf')?'PDF/OCR抽出':'画像OCR抽出'});
        if(asset.allowedCategories?.length)extracted=extracted.filter(x=>asset.allowedCategories.includes(x.category));
        if(store.id==='akachan-aizumi')extracted=extracted.map(x=>({...x,sourceGroup:asset.sourceGroup||'セール・チラシ情報',imageUrl:displayUrl}));
    ...
    if(store.id==='akachan-aizumi'){
      ...
      // アカチャンホンポはアカトク4件＋紙おむつセール全体をOCRするため、代表画像だけに商品を絞り込まない。
